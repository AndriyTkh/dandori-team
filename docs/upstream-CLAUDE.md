<!--
UPSTREAM CONTRACT, PRESERVED FOR MERGE REFERENCE — NOT FORK POLICY.

This is `CLAUDE.md` as it stands in upstream `nitatsuu/Dandori`, copied verbatim and
unmodified below this header. It describes a single-user personal planner and its
minimalism contract, including a closed "What must not exist" list that forbids
collaboration.

The fork's live agent contract is the root `CLAUDE.md`. Where the two disagree, the
root file wins. Keep this file byte-identical to upstream so that upstream merges
into it are trivial and so that fork decisions can always be diffed against the
contract they departed from. See `docs/decisions/ADR-0001-fork-contract.md`.
-->
