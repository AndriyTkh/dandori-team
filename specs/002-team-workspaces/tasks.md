...
- [ ] T031a [data] [sub-of: T031] [in-progress: coder-T031a@wt/sync-cache] Add `updateWorkspace(id, patch: Pick<Workspace,'kind'>)` (read, `touch`, put, `queue()`; no RPC, D-6′) and `currentUserId()` (reads `meta['owner']`, mirrors `isAdmin`) to `src/db/api.ts`; `useCurrentUserId()` to `src/db/hooks.ts`; `tests/setup.ts` afterEach clears `members`. Needed by T043/T049 owner-only guards (A-011, gate 10).
